...
start两个作用 1、开始执行这个程序 2.在首条执行的语句处暂停 打断点 将要执行的语句

next 单步执行 简写为n 将要执行第6行但是还没执行第六行 q退出gdb命令

调试插入排序程序insrtSort_64.c

p/d a d代表着十进制 s的意思是进入被调用函数的内部 单步执行

b命令打断点 c命令继续执行(continue) 进入函数的内部 外部的函数的数组不再有效了

P/d *v@9  c是让程序尽力去跑 等走到断点的时候再停下来
s可以进入到某个函数里面去 当单步执行到某个语句的时候
n 两个单步执行 n步出式的调用 s是步入式的

想观察每一轮循环 应该在循环内部打上断点
c 命令全力让程序去跑遇到断点后终止 start不能省略为s
disas反汇编命令 
p /d * 指向首元素的指针 @元素的个数 d表示十进制
p/d * (v+1)@8  十分灵活
p/x 按照16进制去显示二进制  p/x 3.14 p/x (float)3.14 双精度还是单精度
想看看寄存器的值 p $rip 加上寄存器的名字

TUI图形化模式 text User Interface 

delete用于删除断点
![[Pasted image 20260508153630.png]]

直接按回车会重复上一步的命令

linux中每个文件或者文件夹 都有三个身份属性 ：所有者 所属组 和其他人
文件所有者就是创建这个文件的用户 拥有最高的权限 
系统里每个用户都有唯一的UID 系统靠UID识别所有者 不是靠用户名
如果需要重新编译然后再执行 make clean 和make hello.c 配合使用即可
![[Pasted image 20260508160329.png]]